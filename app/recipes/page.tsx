...
import { PantryManager } from "@/components/air-fryer/pantry-manager"
...
// Cooking methods - extensible for future additions
const COOKING_METHODS = [
  { id: "all", label: "All Recipes", icon: BookMarked },
  { id: "air-fryer", label: "Air Fryer", icon: Flame },
  // Future:
  // { id: "wok", label: "Wok", icon: CookingPot },
  // { id: "pan", label: "Pan", icon: Soup },
] as const
...
export default function RecipesPage() {
  ...
  useEffect(() => {
    if (mounted && airFryerRecipes.length === 0) {
      seedAirFryerRecipes()
    }
  }, [mounted, airFryerRecipes.length])

  // Get suggestions when inventory or recipes change
  useEffect(() => {
    if (mounted && airFryerRecipes.length > 0) {
      getSuggestions()
    }
  }, [mounted, airFryerRecipes.length, inventoryItems.length, getSuggestions])

  const seedAirFryerRecipes = async () => {
    const seedData = getSeedAirFryerRecipes()
    for (const recipeData of seedData) {
      await addAirFryerRecipe(recipeData)
    }
  }
      ...
      <div className="container max-w-6xl space-y-6 p-4 md:p-6 lg:p-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Your recipe collection and cooking tools</p>
          </div>
          {activeMethod === "air-fryer" && (
            <OllamaStatusIndicator variant="badge" />
          )}
        </div>

        {/* Cooking Method Tabs */}
        <Tabs value={activeMethod} onValueChange={(v) => setActiveMethod(v as CookingMethod)} className="w-full">
          ...
          {/* Air Fryer Tab */}
          <TabsContent value="air-fryer" className="mt-6 space-y-4">
            {/* Device Settings */}
